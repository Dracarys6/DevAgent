...
from devagent.diagnosis.models import DiagnosisInput, DiagnosisReportDraft
...
7. 不要输出 report_id、scenario、target 或 evidence；这些权威字段由服务端绑定。
8. summary、finding.statement、recommendation 的 action、rationale、
    verification_steps，以及 missing_evidence 的 needed、reason 必须使用简体中文。
9. 代码标识、文件路径、测试名称、异常名称、evidence 原文、JSON 字段名和枚举值
    保持原文，不要为了中文输出而翻译或改写。
10. 只输出与 DiagnosisReportDraft JSON Schema 匹配的 JSON 对象，不要 Markdown 围栏。"""
