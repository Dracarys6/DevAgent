...
**DevAgent：企业级研发效能 AI Agent Runtime 与多 Agent 协作平台.**
...
2. 实现插件式 ToolRegistry / Agent Skills：支持文件读取、代码搜索、Shell 执行、Git diff、日志查询、CI 结果分析和知识库检索等工具。
...
10. 可选实现 MCP 客户端，作为外部工具生态扩展能力。

### 1.4 求职导向能力地图

本项目最终要证明的不是“调用大模型 API”，而是具备企业级 Agent 工程落地能力。项目能力按六条主线组织：

```text
1. Agent Runtime：多轮推理、工具调用、防失控、结构化运行结果
2. Agent Skills / ToolRegistry：工具协议、Schema、扩展成本、MCP 预留
3. Permission & Safety：高风险工具审批、命令拦截、workspace 边界、Prompt Injection 防护
4. EventBus & Trace：事件流、执行轨迹、任务回放、可观测性
5. RAG / Memory：代码、日志、CI、文档检索，历史案例记忆，上下文压缩
6. Multi-Agent：任务拆解、并发执行、父子 Trace、预算控制、结果汇总
```
...
```text
              ...
              └─────┬──────┘ └─┬───────┬─────┘
                    │          │       │
        ┌───────────▼───┐  ┌───▼───┐ ┌─▼────────────┐
        │ Event Store   │  │Permission│ │ToolRegistry │
        │ Trace / Replay│  │ Manager  │ │Agent Skills │
        └───────────────┘  └───────┘ └──────┬───────┘
                                            │
            ┌───────────────────────────────▼───────────────────────────────┐
            │                            Tools                              │
            ...
            └───────────────────────────────┬───────────────────────────────┘
                                            │
              ┌─────────────────────────────▼─────────────────────────────┐
              │ Memory / Storage / External Systems                       │
              │ PostgreSQL / Vector DB / Git Repo / Logs / CI / FileSystem│
              └───────────────────────────────────────────────────────────┘

              ┌───────────────────────────────────────────────────────────┐
              │ Evaluation                                               │
              │ Tool Hit / Evidence Hit / Latency / Failure Rate          │
              └───────────────────────────────────────────────────────────┘
```
...
#### 第三，Agent Skills 面向业务组合工具

单个工具只解决一次原子操作，Agent Skills 面向研发场景组合工具能力。
...
#### 第六，RAG / Memory 服务诊断质量

RAG 和 Memory 不作为炫技模块，而是服务代码、日志、CI 和文档证据检索。
...
检索相关代码和 Git diff
重排关键片段
压缩为 evidence snippets
Agent 基于 evidence 输出报告
```
...
## 6.8 MultiAgentCoordinator 模块

### 6.8.1 职责

MultiAgentCoordinator 负责父 Agent 与子 Agent 的受控编排。

核心能力：

```text
1. 父 Agent 将复杂任务拆成多个独立子任务。
2. 每个子 Agent 使用独立 messages 和 max_steps。
3. 子 Agent 只获得明确允许的工具集合。
4. 多个子 Agent 可以在并发上限内执行。
5. 父任务取消时，取消所有未完成子任务。
6. 子 Agent 事件通过 parent_task_id 桥接到父任务 Trace。
7. 父 Agent 汇总子任务成功、失败和证据。
8. 限制最大层级，避免无限递归创建子 Agent。
```
...
父 Agent 判断任务可以拆分
  ↓
调用 spawn_agent 创建子任务
  ↓
MultiAgentCoordinator 校验层级、工具和预算
  ↓
在并发上限内运行多个子 Agent
  ↓
发布 ChildAgentStarted / Finished / Failed 事件
  ↓
收集 SubAgentResult
  ↓
父 Agent 基于子任务证据生成最终结论
```

### 6.8.4 安全与防失控

```text
max_depth：默认只允许父 Agent 创建一层子 Agent
max_children：限制单个父任务可创建的子任务数量
max_concurrency：限制同时运行的子 Agent 数量
shared_budget：限制整棵任务树的总工具调用和总耗时
allowed_tools：每个子 Agent 只获得完成任务所需工具
cancel_propagation：父任务取消时向下传播
permission_inheritance：高风险工具仍必须经过 PermissionManager
```
...
子 Agent A：检索并分析日志
子 Agent B：查看相关代码和 Git diff
子 Agent C：分析 CI 结果

父 Agent：汇总各子任务证据，输出根因和修复建议
```
...
```http
GET /api/v1/agent/tasks/{task_id}/children
```
  ...
  "children": [
    {
      "task_id": "task_002",
      "role": "log_analyst",
      "goal": "分析 task_id 对应日志",
      "status": "DONE"
    },
    {
      "task_id": "task_003",
      "role": "code_analyst",
      "goal": "分析相关代码与 Git diff",
      "status": "RUNNING"
    }
  ]
}
...
```http
POST /api/v1/agent/tasks/{task_id}/cancel-tree
```
...
* [x] 实现 `POST /api/v1/diagnoses/log`，通过结构化时间线区分首个异常、连锁错误和最终症状。
* [x] 使用真实 provider 对固定日志 case 运行端到端诊断，保存同口径的结构化报告和失败记录。
...
* [ ] 在 `src/devagent/multi_agent/models.py` 定义 `SubAgentTask`、`SubAgentResult`、`AgentBudget`。
* [ ] 在 `src/devagent/multi_agent/spawn_tool.py` 实现 `SpawnAgentTool`。
* [ ] 在 `src/devagent/multi_agent/coordinator.py` 实现 `spawn()`、`gather()`、`cancel_tree()`。
* [ ] 子 Agent 使用独立上下文和受限工具集合。
* [ ] 支持 `max_depth`、`max_children`、`max_concurrency` 和共享预算。
* [ ] 子 Agent 事件通过 `parent_task_id` 桥接到父 Agent。
* [ ] 父任务取消时向所有运行中的子任务传播取消。
* [ ] 父 Agent 汇总子 Agent 成功、失败和证据。
* [ ] 实现 `GET /api/v1/agent/tasks/{task_id}/children` 查询任务树。
* [ ] 实现 `POST /api/v1/agent/tasks/{task_id}/cancel-tree` 取消任务树。
* [ ] Trace 支持展示父子 Agent 调用关系。
* [ ] 增加多 Agent 集成测试和业务 Demo。
...
```text
1. 父 Agent 可以并发创建日志、代码 diff、CI 三个子任务。
2. 子 Agent 之间上下文隔离，不会互相污染 messages。
3. 子 Agent 只能使用 allowed_tools 中声明的工具。
4. 高风险工具仍会触发 PermissionManager。
5. 父任务取消后，运行中的子任务被取消。
6. 任一子任务失败时，父 Agent 可以基于其他结果部分降级。
7. Trace 可以按 parent_task_id 回放完整任务树。
...
这一章按“先能写 Python 小程序，再做 Agent Runtime，再把业务闭环、评测和可观测性补齐”的顺序安排。不要一开始就同时学习数据库、前端、向量库、多 Agent、MCP，这样很容易被范围压垮。

本章只描述阶段目标。具体到每天的开发文件、类、接口、测试和每周额外收尾，以独立文档 `learning_plan.md` 为准，避免项目设计文档与每日计划重复维护后产生冲突。
...
第 2 阶段：ToolRegistry / Agent Skills / Tool Calling
...
```text
第 1 周：
read_file + search_code + run_shell + ToolResult

第 2 周：
ToolRegistry + Agent Loop + Mock/真实 LLM + CLI

第 3 周：
FastAPI + TaskManager + 任务状态机 + 取消

第 4 周：
PermissionManager + CommandGuard + ToolExecutor

第 5 周：
EventBus + SSE/WebSocket + Trace 回放
