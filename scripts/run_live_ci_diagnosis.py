...
def main() -> None:
    parser = ArgumentParser(
        description="显式调用真实 LLM provider 执行固定 CI 诊断验收"
    )
    ...
    if args.input_json is not None:
        run = LiveCIDiagnosisRun.model_validate_json(
    ...
    markdown = render_live_ci_diagnosis_report(
        run,
    ...
    print(f"Live CI diagnosis report 已生成: {output}")
    print(f"Live CI diagnosis raw result 已生成: {json_output}")
    print(
        "端到端验收: "
